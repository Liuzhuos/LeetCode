...
一开始想到的解题思路就是两次循环遍历，但复杂度有些高，所以选用以下方法：

1. 将其拆分成 values[i]+i 和 values[j]−j 两部分，这样对于统计景点 j 答案的时候，由于 **values[j]−j 是固定不变的**，因此最大化 values[i]+i+values[j]−j 的值其实就**等价于求 [0,j−1] 中 values[i]+i 的最大值 mx**，景点 j 的答案即为 mx+values[j]−j 。
2. 而 mx 的值我们只要从前往后遍历 j 的时候同时维护即可
